...
From a fresh clone, this sets up the whole environment (toolchain, the
`fast_hadamard_transform` CUDA kernel, and the FlatQuant package), downloads a
model + datasets, and runs FlatQuant:

```bash
conda create -n flatquant python=3.10 -y && conda activate flatquant
bash setup.sh                                           # build + install everything
bash scripts/download_assets.sh Qwen/Qwen2.5-7B-Instruct  # model + WikiText-2 + C4
bash scripts/qwen-2.5-instruct/qwen-2.5-instruct-7b/w4a4kv4.sh   # run
```

`setup.sh` is idempotent and works with pip-wheel builds of torch (including
CUDA 13 `cu130` wheels): if `nvcc` is missing it installs a matching
`cuda-toolkit` into the conda env, and it patches `fast_hadamard_transform` for
CUDA 13 (which dropped the `compute_70`/Volta target). By default it installs
the **core** package only; to also build the optional `deploy._CUDA` inference
kernels run `BUILD_DEPLOY_KERNELS=1 bash setup.sh` (needs `cmake` + the `cutlass`
submodule). The commonsense-QA datasets used by `--lm_eval` are fetched
automatically at evaluation time.

The sections below describe the equivalent manual steps.

### Setup notes / local changes

To make the repo build and run on a modern stack (pip-wheel **torch with CUDA 13**,
e.g. `2.x+cu130`, and an environment without a system CUDA toolkit), the
following changes were made. `setup.sh` applies the build-related ones for you.

1. **`setup.py` — optional deploy kernels.** A new env flag
   `FLATQUANT_SKIP_DEPLOY_KERNELS=1` installs the core package without compiling
   the `deploy._CUDA` inference kernels (which need `cmake` + the `cutlass`
   submodule). The core path (calibration/PPL/QA via `main.py`) does not need
   them. Build them with `BUILD_DEPLOY_KERNELS=1 bash setup.sh`.

2. **`flatquant/model_utils.py` — Qwen model detection.** `get_model()` matched
   `'qwen-2.5'`, but the model dir is `Qwen2.5-7B-Instruct`, so the Qwen script
   failed with `Unknown model`. Detection is now case-insensitive and accepts
   both `qwen2.5` and `qwen-2.5`.

3. **`third-party/fast-hadamard-transform/setup.py` — CUDA 13 arch fix
   (submodule).** CUDA 13 dropped the `compute_70` (Volta) target, which the
   upstream kernel hard-codes, causing
   `nvcc fatal: Unsupported gpu architecture 'compute_70'`. The fix gates
   `compute_70` behind `CUDA < 13` and adds `compute_86` (Ampere):

   ```diff
   -    cc_flag.append("-gencode")
   -    cc_flag.append("arch=compute_70,code=sm_70")
   +    # CUDA 13 dropped compute_70 (Volta); only emit arches nvcc supports.
   ...
   Because this lives in a **git submodule** (a separate repository), it cannot
   be committed to this repo. `setup.sh` re-applies it automatically (idempotently)
   on every fresh clone, so you do not need to patch it by hand.

4. **Build isolation.** `pip install -e .` builds in an isolated env that does
   not see your installed `torch`; the editable install therefore uses
   `--no-build-isolation` (handled by `setup.sh`).

5. **Run scripts.** The provided run scripts export
   `HF_DATASETS_TRUST_REMOTE_CODE=1` (so `lm-eval` can fetch the commonsense-QA
   datasets) and `cd` to the repo root, so they run from anywhere.
...
**Note:**
- The editable install uses PEP 517 build isolation by default, which does not
  see your installed `torch`. If `pip install -e .` fails with
  `ModuleNotFoundError: No module named 'torch'`, add `--no-build-isolation`.
- To run models like LLaMA2 and LLaMA3, please install dependencies from `requirements_llama2.txt`.

### Data Preparation

The quickest way is `bash scripts/download_assets.sh <HF_MODEL_REPO>`, which
downloads the model plus the WikiText-2 and C4 datasets used for calibration and
PPL into the layout below. The manual equivalents follow.
